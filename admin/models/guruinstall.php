...
class guruAdminModelguruinstall extends JModelLegacy {
	...
	{
		$this->addIndexTable('#__guru_authors', array(
			'userid',
			'enabled',
			'commission_id',
			'paypal_option',
		));
		$this->addIndexTable('#__guru_authors_commissions', array(
			'author_id',
			'course_id',
			'plan_id',
			'order_id',
		...
		$this->addIndexTable('#__guru_authors_commissions_history', array(
			'author_id',
		));
		$this->addIndexTable('#__guru_buy_courses', array(
			'userid',
			'order_id',
			'course_id',
		));
		$this->addIndexTable('#__guru_category', array(
			'published',
		));
		$this->addIndexTable('#__guru_categoryrel', array(
			'parent_id',
			'child_id',
		));
		$this->addIndexTable('#__guru_certificates', array(
			'course_id',
		));
		$this->addIndexTable('#__guru_days', array(
			'pid',
			'published',
		));
		$this->addIndexTable('#__guru_emails_pending', array(
			'mail_id',
			'pid',
			'send',
		));
		$this->addIndexTable('#__guru_fields', array(
			'group_id',
			'published',
		));
		$this->addIndexTable('#__guru_fields_results', array(
			'user_id',
			'field_id',
		));
		$this->addIndexTable('#__guru_groups', array(
			'published',
		));
		$this->addIndexTable('#__guru_jomsocialstream', array(
			'userid',
		));
		$this->addIndexTable('#__guru_kunena_courseslinkage', array(
			'idcourse',
			'catidkunena',
		));
		$this->addIndexTable('#__guru_kunena_lessonslinkage', array(
			'idlesson',
			'catidkunena',
		));
		$this->addIndexTable('#__guru_logs', array(
			'userid',
			'productid',
			'emailid',
		));
		$this->addIndexTable('#__guru_media', array(
			'type',
			'uploaded',
			'author',
			'published',
		));
		$this->addIndexTable('#__guru_mediarel', array(
			'type',
			'type_id',
			'media_id',
			'mainmedia',
		...
		$this->addIndexTable('#__guru_media_categories', array(
			'parent_id',
			'child_id',
			'published',
			'user_id',
		));
		$this->addIndexTable('#__guru_mycertificates', array(
			'course_id',
			'author_id',
			'user_id',
			'completed',
		));
		$this->addIndexTable('#__guru_order', array(
			'userid',
			'status',
			'published',
			'order_date',
		));
		$this->addIndexTable('#__guru_plugin_settings', array(
			'pluginid',
		));
		$this->addIndexTable('#__guru_program', array(
			'catid',
			'published',
		));
		$this->addIndexTable('#__guru_programstatus', array(
			'userid',
			'pid',
		));
		$this->addIndexTable('#__guru_program_plans', array(
			'product_id',
			'plan_id',
			'default',
		));
		$this->addIndexTable('#__guru_program_reminders', array(
			'product_id',
			'emailreminder_id',
			'send',
		));
		$this->addIndexTable('#__guru_program_renewals', array(
			'product_id',
			'plan_id',
			'default',
		));
		$this->addIndexTable('#__guru_projects', array(
			'course_id',
			'author_id',
			'published',
		));
		$this->addIndexTable('#__guru_project_results', array(
			'project_id',
			'student_id',
			'course_id',
			'lesson_id',
		));
		$this->addIndexTable('#__guru_promos', array(
			'published',
		));
		$this->addIndexTable('#__guru_questions', array(
			'qid',
			'published',
		));
		$this->addIndexTable('#__guru_questions_v3', array(
			'qid',
			'type',
			'published',
		));
		$this->addIndexTable('#__guru_question_answers', array(
			'question_id',
		));
		$this->addIndexTable('#__guru_quiz', array(
			'published',
			'author',
			'is_final',
		));
		$this->addIndexTable('#__guru_quizzes_final', array(
			'qid',
			'published',
		));
		$this->addIndexTable('#__guru_quiz_essay_mark', array(
			'question_id',
			'user_id',
		));
		$this->addIndexTable('#__guru_quiz_question_taken', array(
			'user_id',
			'question_id',
		));
		$this->addIndexTable('#__guru_quiz_question_taken_v3', array(
			'user_id',
			'quiz_id',
			'pid',
		));
		$this->addIndexTable('#__guru_quiz_taken', array(
			'user_id',
			'quiz_id',
			'pid',
		));
		$this->addIndexTable('#__guru_quiz_taken_v3', array(
			'user_id',
			'quiz_id',
			'pid',
			'question_id',
		...
		$this->addIndexTable('#__guru_scorm', array(
			'lesson_id',
		));
		$this->addIndexTable('#__guru_scorm_items', array(
			'course_id',
			'author_id',
			'published',
		));
		$this->addIndexTable('#__guru_scorm_scoes', array(
			'scorm_id',
		));
		$this->addIndexTable('#__guru_scorm_scoes_data', array(
			'scorm_id',
		));
		$this->addIndexTable('#__guru_subplan', array(
			'published',
		));
		$this->addIndexTable('#__guru_subremind', array(
			'published',
		));
		$this->addIndexTable('#__guru_task', array(
			'published',
		));
		$this->addIndexTable('#__guru_taskcategory', array(
			'published',
		));
		$this->addIndexTable('#__guru_taskcategoryrel', array(
			'parent_id',
			'child_id',
		));
		$this->addIndexTable('#__guru_task_kunenacomment', array(
			'id_lesson',
			'thread',
		));
		$this->addIndexTable('#__guru_viewed_lesson', array(
			'user_id',
			'completed',
			'pid',
		));
		$this->addIndexTable('#__guru_viewed_lesson_time', array(
			'user_id',
			'lesson_id',
		));
	...
	protected function addIndexTable($tableName, $indexes)
	{
		$db = JFactory::getDbo();
		$query = "SHOW INDEX FROM #__guru_mediarel;";
		$results = $db->setQuery($query)->loadObjectList();
		$indexed = array_map(function($results) {
