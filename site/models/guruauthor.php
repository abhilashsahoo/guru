...
class guruModelguruauthor extends JModelList
{
	...
	function getAuthorMyEssays()
	{
		...
		$subQuery = "SELECT q.id FROM `#__guru_questions_v3` q WHERE 1=1 " . $and_essays_filter . " AND q.type='essay' AND q.id IN (" . implode(",", $quizzes_ids) . ")";
		...
		$query = 
			"SELECT 
				...
				c.image, 
				em.grade, 
				em.feedback_quiz_results, 
				qt3.question_id, 
				qt3.answers_given, 
				qt3.pid,
				qt3.quiz_id AS qid
			FROM `#__guru_quiz_taken_v3` qt3
			LEFT JOIN `#__users` u ON qt3.user_id = u.id
			LEFT JOIN `#__guru_customer` c ON qt3.user_id = c.id
			LEFT JOIN `#__guru_questions_v3` q ON qt3.question_id = q.id
			LEFT JOIN `#__guru_quiz_essay_mark` em ON qt3.question_id = em.question_id
			WHERE qt3.question_id IN ($subQuery) 
			$and_course_filter
			$and_status_filter
			$and_search_filter
			AND qt3.pid IN (". implode(',', $authors_courses) .")
